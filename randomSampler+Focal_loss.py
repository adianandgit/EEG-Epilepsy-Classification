...
BEST_VAL_F1 = -1.0
BEST_MODEL_PATH = "best_f1_model.pt"
FINAL_MODEL_PATH = "final_f1_model.pt"
...
class PartialUndersamplingDataset(Dataset):
    def __init__(self, pt_file_path, augment=False, ratio=5):
        ...
        self.augment, self.jitter_sigma, self.scale_factor = augment, 0.03, 0.15
...
alpha_weights = torch.tensor([0.25, 0.75], dtype=torch.float32).to(DEVICE)
criterion = FocalLoss(alpha=alpha_weights)
print(f"Using FocalLoss with alpha={alpha_weights.cpu().numpy()}")
