...
BEST_VAL_F1 = -1.0
BEST_MODEL_PATH = "best_minimal_model.pt"
FINAL_MODEL_PATH = "final_minimal_model.pt"
...
print("Pretrained weights loaded and backbone frozen.")

print("Preparing datasets...")
train_ds = EEGDataset(os.path.join(DATA_PATH, "train.pt"), augment=True)
valid_ds = EEGDataset(os.path.join(DATA_PATH, "val.pt"))
test_ds = EEGDataset(os.path.join(DATA_PATH, "test.pt"))

print(f"Train label counts: {np.bincount(train_ds.labels)}")
train_dl = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True)
valid_dl = DataLoader(valid_ds, batch_size=BATCH_SIZE)
test_dl = DataLoader(test_ds, batch_size=BATCH_SIZE)

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(list(model.stem.parameters()) + list(model.feedforward_net.parameters()),
...
def evaluate(model, dl, print_report=False):
    ...
    with torch.no_grad():
        for x, y in dl:
            ...
            all_p.extend(torch.max(out, 1)[1].cpu().numpy())
            all_s.extend(F.softmax(out, dim=1)[:, 1].cpu().numpy())
    target_names = ["Normal", "Abnormal"]
    report = classification_report(all_y, all_p, output_dict=True, zero_division=0, target_names=target_names)
    if print_report:
        print(classification_report(all_y, all_p, zero_division=0, target_names=target_names))
    f1_class_1 = report.get("Abnormal", {}).get("f1-score", 0.0)
    ...
    try:
        ...
        auc = 0.5
    return acc * 100, f1_class_1, cm, auc, all_y, all_p, all_s

print("Training started.")
for epoch in range(EPOCHS):
    loss = train_epoch(model, train_dl, optimizer)
    acc, f1, _, auc, _, _, _ = evaluate(model, valid_dl)
    scheduler.step(f1)
    history['train_loss'].append(loss)
    history['val_acc'].append(acc)
    history['val_f1_class1'].append(f1)
    history['val_auc'].append(auc)
    print(f"Epoch {epoch+1}/{EPOCHS}: Loss={loss:.4f}, Val Acc={acc:.2f}%, Val F1={f1:.4f}, AUROC={auc:.4f}, LR={optimizer.param_groups[0]['lr']:.2e}")
    if f1 > BEST_VAL_F1:
        BEST_VAL_F1 = f1
        torch.save(model.state_dict(), BEST_MODEL_PATH)
        print(f"New best model saved (F1={f1:.4f})")

print(f"\nFinal Evaluation (Best Val F1={BEST_VAL_F1:.4f})")
if os.path.exists(BEST_MODEL_PATH):
    model.load_state_dict(torch.load(BEST_MODEL_PATH))
else:
    print("No best model found; evaluating final weights.")
...
plt.figure()
plt.plot(history['val_f1_class1'])
plt.title('Validation F1')
plt.savefig(os.path.join(PLOT_PATH, 'f1.png'))
plt.close()

plt.figure()
plt.plot(history['val_auc'])
plt.title('Validation AUROC')
plt.savefig(os.path.join(PLOT_PATH, 'auc.png'))
plt.close()
